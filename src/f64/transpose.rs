...
#[inline(always)]
fn untranspose_2d<const M: usize>(rows: [[f64; M]; 2]) -> [[f64; 2]; M]
where
    LaneCount<M>: SupportedLaneCount,
{
...
#[inline(always)]
fn untranspose_3d_2(cols: [[f64; 2]; 3]) -> [[f64; 3]; 2] {
    let mut data = [0.0; 6];
...
#[inline(always)]
fn untranspose_3d_4(cols: [[f64; 4]; 3]) -> [[f64; 3]; 4] {
    let mut data = [0.0; 12];
    for (i, &x) in cols.as_flattened().iter().enumerate() {
        data[i] = x;
    }
...
#[inline(always)]
fn untranspose_3d_8(cols: [[f64; 8]; 3]) -> [[f64; 3]; 8] {
    let mut data = [0.0; 24];
    for (i, &x) in cols.as_flattened().iter().enumerate() {
        data[i] = x;
    }
...
#[inline(always)]
fn untranspose_3d_16(cols: [[f64; 16]; 3]) -> [[f64; 3]; 16] {
    let mut data = [0.0; 48];
    for (i, &x) in cols.as_flattened().iter().enumerate() {
        data[i] = x;
    }
    ...
    LaneCount<M>: SupportedLaneCount,
{
    match D {
        2 => {
            let cols: &[[f64; M]; 2] = unsafe { transmute(&cols) };
            let rows = untranspose_2d(*cols);
            let rows: &[[f64; D]; M] = unsafe { transmute(&rows) };
            return *rows;
        }
        3 => match M {
            2 => {
                let cols: &[[f64; 2]; 3] = unsafe { transmute(&cols) };
                let rows = untranspose_3d_2(*cols);
                let rows: &[[f64; D]; M] = unsafe { transmute(&rows) };
                return *rows;
            }
            4 => {
                let cols: &[[f64; 4]; 3] = unsafe { transmute(&cols) };
                let rows = untranspose_3d_4(*cols);
                let rows: &[[f64; D]; M] = unsafe { transmute(&rows) };
                return *rows;
            }
            8 => {
                let cols: &[[f64; 8]; 3] = unsafe { transmute(&cols) };
                let rows = untranspose_3d_8(*cols);
                let rows: &[[f64; D]; M] = unsafe { transmute(&rows) };
                return *rows;
            }
            16 => {
                let cols: &[[f64; 16]; 3] = unsafe { transmute(&cols) };
